...

{
  options = {
    services.nginx = {
      ...
      sslSecurityDhparam = mkOption {
        type = types.nullOr types.str;
        default = null;
        example = "nginx";
        description = "Use dhparams generated by <option>security.dhparams.params.\${sslSecurityDhparam}</option>.";
      };
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.nginx = {
      ...
      wants = mkMerge [
        (concatLists (map (certName: [ "acme-finished-${certName}.target" ]) dependentCertNames))
        (mkIf (cfg.sslSecurityDhparam != null && config.security.dhparams.stateful) [ "dhparams-gen-${cfg.sslSecurityDhparam}.service" ])
      ];
      after = mkMerge [
        ([ "network.target" ] ++ map (certName: "acme-selfsigned-${certName}.service") dependentCertNames)
        (mkIf (cfg.sslSecurityDhparam != null && config.security.dhparams.stateful) [ "dhparams-gen-${cfg.sslSecurityDhparam}.service" ])
      ];
    ...
    security.dhparams = mkIf (cfg.sslSecurityDhparam != null) {
      enable = true;
      params."${cfg.sslSecurityDhparam}" = { };
    };

    services.nginx.sslDhparam = mkIf (cfg.sslSecurityDhparam != null)
      (mkDerivedConfig opts.sslSecurityDhparam (o: config.security.dhparams.params.${o}.path));
