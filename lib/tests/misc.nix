...
# CLI
  ...
  testMakeOverridable =
    let
      ...
      pkgFix = makeOverridable ({ a, b }: fix (self: mkDerivation (pkgArgs { inherit self a b; })));
      ...
      pkgSelf = makeOverridable ({ _autoSelf ? null, self, a, b } @ args: mkDerivation (pkgArgs args));
